...
frappe.ui.form.on("Delivery Note", {
	...
	scan_rfid_button: function (frm) {
		if (frm.doc.ip_address) {
			...
			frappe.show_alert({ message: "Vui lòng chọn máy đọc RFID!", indicator: "red" });
		}
	},
	stop_rfid_button: function (frm) {
		if (frm.doc.ip_address) {
	...
	clear_cache_rfid_button: function (frm) {
		if (frm.doc.ip_address) {
	...
	scan_RFID(frm, module_path) {
		if (!frm.doc.rfid_reader_name) {
			frappe.show_alert({ message: "Vui lòng chọn máy để quét...!", indicator: "red" });
			return;
		}
					...
					const success_cb = () => {
						...
						start_getting_rfid(frm, (epcArray) => {
							...
							epcArray.forEach((rfid) => {
								...
								const selling_rule =
									has_item && item_found?.warehouse == rfid_found?.warehouse;

								if (selling_rule && rfid_found?.tag === rfid && !seenTags.has(rfid)) {
	...
	clear_cache_RFID(frm, module_path) {
		...
		const child_item = frm.doc.items[0];

		frappe.require(module_path).then(() => {
			success_cb = (data) => {
				frappe.model.set_value(child_item.doctype, child_item.name, "serial_no", "");
				frappe.model.set_value(child_item.doctype, child_item.name, "qty", 0);
...
frappe.ui.form.on("RFID Ledger Detail", {
	rfid_tag(frm, cdt, cdn) {
		frm.doc.items.forEach((res) => {
			...
			frappe.model.set_value(res.doctype, res.name, "serial_no", rfids.join("\n"));
		});
	},
	rfids_remove: function (frm, cdt, cdn) {
		frm.doc.items.forEach((res) => {
			const rfids = frm.doc.rfids.filter((r) => r.item === res.item_code).map((e) => e.rfid_tag);
			frappe.model.set_value(res.doctype, res.name, "serial_no", rfids.join("\n"));
			frappe.model.set_value(res.doctype, res.name, "qty", rfids.length);
		});
	},
});
