...
class StockEntry(StockController):
	...
	def update_rfids_location(self):
		for item in self.items:
			rfids = [rfid.rfid_tag for rfid in self.rfids if rfid.item == item.item_code]

			for rfid_tag in rfids:
				try:
					rfid_doc = frappe.get_doc("RFID Tag", rfid_tag)
					rfid_doc.warehouse = item.t_warehouse
					rfid_doc.customer = None  # hoặc False tùy kiểu field
					rfid_doc.save()
				except frappe.DoesNotExistError:
					frappe.msgprint(f"RFID Tag {rfid_tag} không tồn tại.")
